	...
	FreeRectChoiceHeuristic rectChoice, GuillotineSplitHeuristic splitMethod)
{
	...
	// Pack rectangles one at a time until we have cleared the images array of all rectangles.
	// images will get destroyed in the process.
	while(rects.size() > 0)
	{
...
void CountNumFitting(const Rect &freeRect, int width, int height, const std::vector<RectSize> &images,
	int usedRectIndex, bool splitHorizontal, int &score1, int &score2)
{
	const int width = freeRect.width - width;
	const int height = freeRect.height - height;
	...
	for(size_t i = 0; i < images.size(); ++i)
		if (i != usedRectIndex)
		{
			if (FitsPerfectly(images[i], bottom))
				fitBottom |= 0x10000000;
			if (FitsPerfectly(images[i], right))
				fitRight |= 0x10000000;

			if (Fits(images[i], bottom))
				++fitBottom;
			if (Fits(images[i], right))
				++fitRight;
		}
...
*/
/*
// Implements GUILLOTINE-MAXFITTING, an experimental method that's really cool but didn't quite work in practice.
void GuillotineBinPack::InsertMaxFitting(std::vector<RectSize> &images, std::vector<Rect> &dst, bool merge,
	FreeRectChoiceHeuristic rectChoice, GuillotineSplitHeuristic splitMethod)
{
	dst.clear();
	int bestRect = 0;
	...
	while(images.size() > 0 && freeRectangles.size() > 0)
	{
		int bestScore1 = -1;
		int bestScore2 = -1;
		...
		for(size_t j = 0; j < images.size(); ++j)
		{
			...
			if (images[j].width == freeRect.width && images[j].height == freeRect.height)
			{
				bestRect = j;
				bestFlipped = false;
			...
			else if (images[j].width <= freeRect.width && images[j].height <= freeRect.height)
			{
				CountNumFitting(freeRect, images[j].width, images[j].height, images, j, false, score1, score2);
				...
				CountNumFitting(freeRect, images[j].width, images[j].height, images, j, true, score1, score2);
			...
			if (images[j].height == freeRect.width && images[j].width == freeRect.height)
			{
				bestRect = j;
				bestFlipped = true;
				bestScore1 = bestScore2 = std::numeric_limits<int>::max();
			...
			else if (images[j].height <= freeRect.width && images[j].width <= freeRect.height)
			{
				CountNumFitting(freeRect, images[j].height, images[j].width, images, j, false, score1, score2);
				...
				CountNumFitting(freeRect, images[j].height, images[j].width, images, j, true, score1, score2);
		...
		if (bestScore1 >= 0)
		{
			...
			newNode.width = images[bestRect].width;
			newNode.height = images[bestRect].height;
			if (bestFlipped)
				std::swap(newNode.width, newNode.height);
			...
			images.erase(images.begin() + bestRect);
...
void GuillotineBinPack::SplitFreeRectByHeuristic(const Rect &freeRect, const Rect &placedRect, GuillotineSplitHeuristic method)
{
	...
	// Use the given method to decide which choice to make.
